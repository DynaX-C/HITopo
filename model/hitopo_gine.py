...
from torch_geometric.nn import GINEConv, global_add_pool, Linear

class HITopo_GINE(torch.nn.Module):
    """
    Hierarchical interaction topology with GINE backbone.

    Args:
        node_features_dim (int): input node feature dimension.
        hidden_channels (int): hidden dimension of all graph representations.
        dropout (float): dropout rate in MLP blocks.
        num_layers (int): number of GINE layers.
        eps (float): initial epsilon value in GINE aggregation.
        train_eps (bool): whether epsilon is learnable.
        edge_dim (int): dimension of edge features (used in message passing).
    ...
    def forward(self, data):
        ...
        for l, (nb_conv, bd_conv, bond_conv, angle_conv) in enumerate(
                zip(self.nb_convs, self.bd_convs, self.bond_convs, self.angle_convs)):
            atom_x_prev = atom_x
            if self.separate_nb_bd:
                if self.nb_only:
            ...
            if self.use_high_order_graph:
                ...
                bond_x_self = bond_conv(bond_x_prev, bond_edge_index, bond_edge_attr) + bond_x_prev
                angle_x_self = angle_conv(angle_x_prev, angle_edge_index, angle_edge_attr) + angle_x_prev
    ...
    def bond_to_atom_message(self, bond_x, edge_index_intra, num_nodes, use_deg_norm: bool = False,):
    ...
    def angle_to_atom_message(self, angle_x, angle_face, num_nodes, use_deg_norm: bool = False,):
        a, b, c = angle_face
    ...
    def angle_to_bond_message(self, angle_x, angle_to_bond_index, num_bonds, use_deg_norm: bool = False,):
